...
namespace Kuga\Core\Acc\Service;
...
class Acc extends AbstractService
{
    ...
    public function sortRolesPriority($ids)
    {
        if (is_array($ids) && ! empty($ids)) {
            $i         = 0;
            $model     = new RoleModel();
            $modelName = '\\Kuga\\Core\\Acc\\Model\\RoleModel';
            foreach ($ids as $id) {
    ...
    public function assignMenuToRole($menu, $roleId)
    {
        ...
        try {
            $roleId    = intval($roleId);
            $modelName = "\\Kuga\\Core\\Acc\\Model\\RoleMenuModel";
    ...
    public function unassginResourceToRole($roleId, $resourceCode)
    {
        ...
        $modelName = '\\Kuga\Core\Acc\Model\RoleResModel';
    ...
    {
        $userRoleModelName = '\\Kuga\\Core\\Acc\\Model\\RoleUserModel';
        $roleModelName     = '\\Kuga\\Core\\Acc\\Model\\RoleModel';
        $roleModel         = new RoleModel();
